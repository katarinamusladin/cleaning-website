...
?>
<h1 align="center">EDIT A BLOG POST<h1>

<?php
$idp=$_GET["idp"];
		  $upit="select * from blog where id=".$idp.";";
		  $result = $conn->query($upit);
		  $row = $result->fetch_assoc();
?>
	<img src="../wwwStvarna/<?php  echo $row["slika"] ?>" width="400" height="300">
<form action="nova.php?id=<?php echo $row['id'] ?>" method="post" enctype="multipart/form-data">
...
?>
<br><br>
<form action="novaBlog.php?id=<?php echo $row['id'] ?>" method="post">
  <label for="fname">Title: </label><br>
    <input type="text" name="nnaslov" value="<?php echo $row['naslov'] ?>" size="200"><br>
<br>

  <label for="fname">Short description of an article (it shows on main page):</label><br>
  <textarea  id="editor2" name="nopis" autofocus>
  <?php
  echo $row["opis"];
  ?>
	</textarea><br>
  ...
  <label for="fname">Article:  </label><br>
  <textarea  id="editor1" name="ntekst" autofocus>
  <?php
  echo $row["post"];
  ?>
	</textarea><br>
